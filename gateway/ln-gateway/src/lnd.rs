...
impl ILnRpcClient for GatewayLndClient {
    ...
    async fn pay(
        ...
        info!("LND got client to pay invoice {invoice:?}, will check if payment already exists");
            ...
            .await?
        {
            ...
            info!("LND payment does not exist for invoice {invoice:?}, will attempt to pay");
            let payments = client
                .router()
            ...
            loop {
                match messages.message().await.map_err(|error| {
                    ...
                    Some(payment) if payment.status() == PaymentStatus::InFlight => {
                        info!("LND payment is inflight");
                        continue;
                    }
                    Some(payment) => {
