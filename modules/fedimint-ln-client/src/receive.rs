...
impl LightningReceiveConfirmedInvoice {
    ...
    async fn await_incoming_contract_account(
        ...
        loop {
            ...
            match try_get_decrypted_preimage_status(&global_context, contract_id).await {
                Ok((incoming_contract_account, status)) => {
                    match (incoming_contract_account, status) {
                        (_, DecryptedPreimageStatus::Pending) => {
                            ...
                            if has_invoice_expired(&invoice, now_epoch, CLOCK_SKEW_TOLERANCE) {
                                ...
                                debug!(
                                    "Still waiting preimage decryption for contract {contract_id}"
                                );
                            }
                        }
                        (None, other) => {
                            error!(
                                "Unexpected preimage status {other:?} for contract {contract_id}",
                            );
                            return Err(LightningReceiveError::InvalidPreimage);
                        }
                        (Some(incoming_contract_account), DecryptedPreimageStatus::Some(_)) => {
                            return Ok(incoming_contract_account)
                        }
                        (_, DecryptedPreimageStatus::Invalid) => {
                            return Err(LightningReceiveError::InvalidPreimage)
                        }
                    }
                }
...
async fn try_get_decrypted_preimage_status(
    global_context: &DynGlobalClientContext,
    contract_id: fedimint_ln_common::contracts::ContractId,
) -> Result<
    (Option<IncomingContractAccount>, DecryptedPreimageStatus),
    fedimint_core::api::FederationError,
> {
    // FIXME: we should know if the server supports the v2 api and only call it if
    // it does
    match global_context
        .module_api()
        .get_decrypted_preimage_status_v2(contract_id)
        .await
    {
        Ok(result) => Ok(result),
        Err(_e) => {
            // TODO: check error type?
            let (contract, status) = global_context
                .module_api()
                .get_decrypted_preimage_status(contract_id)
                .await?;
            Ok((Some(contract), status))
        }
    }
}
