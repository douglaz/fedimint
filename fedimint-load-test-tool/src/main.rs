...
enum Command {
    ...
    #[command(
        about = "Run a load where many users in parallel will receive then send a payment through lightning"
    )]
...
async fn print_coordinator_notes(coordinator: &Client) -> anyhow::Result<()> {
    info!("Note summary:");
...
async fn get_coordinator_client(
    ...
    let (client, invite_code) = if let Some(db_path) = db_path {
        ...
        if coordinator_db.exists() {
            let (client, _) = build_client(None, Some(&coordinator_db)).await?;
            (client, invite_code.to_owned())
        } else {
...
#[allow(clippy::too_many_arguments)]
async fn run_ln_ping_pong_load_test(
    archive_dir: Option<PathBuf>,
    users: u16,
    invite_code: Option<InviteCode>,
    initial_notes: Option<OOBNotes>,
...
async fn do_ln_circular_test_user_task(
    ...
    match strategy {
        LnCircularStrategy::TwoGateways => {
            let mut invoice_generation = LnInvoiceGeneration::ClnLightningCli;
        ...
        LnCircularStrategy::SelfPayment => {
            while still_ontime().await {
                do_self_payment(&client, invoice_amount, &event_sender).await?;
                sleep_a_bit().await;
            }
        }
        LnCircularStrategy::PartnerPingPong => {
            ...
            while still_ontime().await {
                do_partner_ping_pong(&client, &partner, invoice_amount, &event_sender).await?;
                sleep_a_bit().await;
            }
        }
    }
...
async fn run_two_gateways_strategy(
    ...
    match *invoice_generation {
        LnInvoiceGeneration::ClnLightningCli => {
            ...
            wait_invoice_payment(client, operation_id, event_sender, pay_invoice_time).await?;
...
async fn do_partner_ping_pong(
    ...
    let pay_invoice_time = fedimint_core::time::now();
    client.pay_bolt11_invoice(invoice).await?;
    wait_invoice_payment(partner, operation_id, event_sender, pay_invoice_time).await?;
